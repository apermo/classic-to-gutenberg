...
-- Uses REPLACE INTO so the file can be re-imported to reset state.

-- Clean up revisions and meta from previous test runs.
DELETE FROM wp_postmeta WHERE post_id IN (100,101,102,103,104,105,106,110,120,121);
DELETE FROM wp_posts WHERE post_parent IN (100,101,102,103,104,105,106,110,120,121) AND post_type = 'revision';

-- Post 100: Simple paragraphs (raw text, relies on wpautop)
REPLACE INTO wp_posts (ID, post_author, post_date, post_date_gmt, post_content, post_title, post_status, post_name, post_type, post_modified, post_modified_gmt)
